...
import {
  openDb, recordAttempt, recordGrade, gradingLog, setStar, recordView, recentViews,
  dueItems, allStates, attemptsFor, stats, activity, streak,
} from './lib/db.mjs';
import { conceptMastery, weakEdges, recommend, frontier } from './lib/mastery.mjs';
import { abilityReport, suggest, TARGET_P } from './lib/ability.mjs';
import { conceptLevels, levelOf, assessmentPasses, rollup, POINTS,
         LEVEL_LABEL, nextStep, PROFICIENT_AT, FAMILIAR_AT } from './lib/levels.mjs';
import { buildAssessment, buildMasteryChallenge, challengeStatus } from './lib/assessment.mjs';
import { buildPlan, writeSession } from './lib/cadence.mjs';
import { appendActivity } from './lib/activity.mjs';
import { nextSet } from './lib/nextset.mjs';
import { gradeAttempt, parsePracticeRequest, jevEnabled } from './lib/jev.mjs';
import { checkAnswer, outcomeFor } from './lib/answercheck.mjs';
...
// Exercises re-read from the page image by scripts/repair_text.py. Their stored
// `garble` score describes text that has since been replaced.
...
async function loadTags() {
  try {
    ...
    exerciseTags = raw.exercises ?? {};
    tagVocabulary = [...new Set(Object.values(raw.vocab ?? {}).flat())].sort();
...
async function loadGraph() {
  try {
    ...
    // Exercise -> concept, so a tag match can be turned into something the study
    // queue understands. Rebuilt with the graph because it is derived from it.
  ...
  if (req.method === 'GET') {
    switch (true) {
      ...
      case p === '/next-set':
        ...
        return json(res, nextSet(db, graph, {
      ...
      case p === '/challenge': {
        ...
        if (url.searchParams.get('peek') === '1') {
          ...
          return json(res, { ...st, pool: undefined, pool_size: st.pool.length });
        }
      ...
      case p === '/assessment': {
        ...
        if (!book) return json(res, { error: 'book required' }, 400);
      ...
      case p === '/course': {
        ...
        if (!meta) return json(res, { error: 'no such book' }, 404);
      ...
      case p.startsWith('/concept/'): {
        ...
        if (!node) return json(res, { error: 'no such concept' }, 404);
        const m = conceptMastery(db);
        const brief = (cid) => {
      ...
      case p.startsWith('/extras/'): {
        ...
        if (putnamExtras.has(id)) {
          return json(res, { ...putnamExtras.get(id), solution_source: 'published' });
        }
        ...
        return json(res, inlineSolution
      ...
      case p === '/search': {
        ...
        if (q.length < 2) return json(res, { q, results: [] });
        ...
        return json(res, { q, results: out.slice(0, limit), tagged: Boolean(tagVocabulary.length) });
      }
  ...
  if (req.method === 'POST') {
    ...
    switch (p) {
      case '/attempt': {
        ...
        return json(res, { ...state, outcome: effectiveOutcome,
          exact: exact ? { ...exact, outcome: effectiveOutcome } : null });
      }
      case '/check-answer': {
        const item = graph.exercises.find((e) => e.id === body.item_id);
        if (!item?.auto_gradable) return json(res, { decided: false });
      ...
      case '/grade-answer': {
        ...
        if (!item || item.auto_gradable || !body.answer?.trim()) {
          return json(res, { decided: false });
        }
        if (!jevEnabled()) return json(res, { decided: false, reason: 'jev_unavailable' });
        ...
        if (!grade?.confident) return json(res, {
          decided: false,
          grade: grade ? { ...grade, feedback: feedbackForGrade(grade) } : null,
        });
        return json(res, { decided: true, ...grade, feedback: feedbackForGrade(grade) });
      }
      case '/practice-request': {
        ...
        if (!jevEnabled() && fallbackSkills.length) {
          const fallback = new URLSearchParams({ kind: 'drill',
            ...
            count: '5', label: text });
          return json(res, { understood: true,
        ...
        if (!jevEnabled()) {
          return json(res, { error: 'no TYPESAFE_API_KEY — the box needs it' }, 503);
        }
        ...
        if (!parsed) return json(res, { error: 'could not reach the model' }, 502);
        ...
        if (!parsed.understood) {
          return json(res, { understood: false, parsed,
