...
/** The problems you have worked most recently, one row per problem: its latest
 *  outcome, how many times you have tried it, and whether any try was a solve.
 *  Generated drills are left out — they are regenerated, not revisited. */
export const recentAttempts = (db, n = 12) =>
  db.prepare(
    `SELECT item_id, item_type, concept_id, MAX(ts) AS ts, COUNT(*) AS tries,
            MAX(outcome = 'solved') AS solved_ever,
            (SELECT outcome FROM attempt b WHERE b.item_id = a.item_id
             ORDER BY ts DESC LIMIT 1) AS outcome
     FROM attempt a WHERE item_type != 'drill'
     GROUP BY item_id ORDER BY ts DESC LIMIT ?`
  ).all(n);
