 ...
 *
 * Ranking alone made the queue a fixed list: the best-fitting problem is the
 * best-fitting problem every time you ask, so skipping one only guaranteed
 * seeing it again. Two things break that. Anything shown recently carries a
 * penalty that decays over about a fortnight, and the queue is *sampled* from
 * the well-fitting band rather than sliced off the top of it — same difficulty,
 * different problems.
 */
// Where to aim, relative to the 85% point. These are Elo points: +200 is roughly
// the difference between a problem you clear 85% of the time and one you clear
// about half the time, which is what "harder" should mean and not more.
export const DIFFICULTY = {
  easier:  -200,   // ~95% - consolidation, or a way back in after a bad run
  target:     0,   // the Eighty Five Percent Rule
  harder:   200,   // ~60%
  stretch:  400,   // ~35% - contest pace, where failing is the normal outcome
};

// Adaptive mode passes its own offset instead of naming a rung. Clamped so a bad
// run cannot drive the queue into trivia, nor a good one into the unanswerable.
export const SHIFT_MIN = -300, SHIFT_MAX = 500;

export function suggest(db, graph, {
  domains: wantedDomains = null, limit = 10, sources = null, includeSolved = false,
  hasText = null, isRepaired = null, exclude = null, sample = true, concepts = null,
  difficulty = 'target', shift: rawShift = null,
  // A general veto on individual exercises, in the same shape as `hasText`.
  // The caller knows things this module should not have to: which exercises the
  // semantic pass judged unservable, and what difficulty ceiling the learner
  // actually asked for.
  allow = null,
} = {}) {
  // An explicit offset wins over a named rung: that is how adaptive mode steers.
  const shift = rawShift === null || Number.isNaN(rawShift)
    ? (DIFFICULTY[difficulty] ?? 0)
    : Math.max(SHIFT_MIN, Math.min(SHIFT_MAX, rawShift));
  const wanted = wantedDomains && wantedDomains.length ? new Set(wantedDomains) : null;
  const wantedConcepts = concepts?.length ? new Set(concepts) : null;
  ...
  const views = new Map(db.prepare('SELECT item_id, ts, views FROM view_event').all()
    .map((v) => [v.item_id, v]));
  ...
  const scored = [];
  for (const e of graph.exercises) {
    if (wanted && !wanted.has(e.domain)) continue;
    if (sources && !sources.includes(e.book_id)) continue;
    if (wantedConcepts
        && !wantedConcepts.has(e.concept_id)
        && !(e.topic_concepts ?? []).some((id) => wantedConcepts.has(id))) continue;
    if (!includeSolved && states.get(e.id)?.status === 'solved') continue;
    if (skip?.has(e.id)) continue;
    if (allow && !allow(e)) continue;
    // `garble` scores the *original* extraction. An exercise re-read from the
    // page image has been replaced since, so the old score no longer describes
    // the text we would serve — without this, repairing a book would change
    // nothing about what it offers you.
    if (e.ocr && (e.garble ?? 0) > 0.3 && !(isRepaired && isRepaired(e.id))) continue;
    // Exercises from copyrighted books carry no text in the graph — it lives in
    // data/local/. Without this check the queue could only ever offer the two
    // sources whose text is committed, silently ignoring the other seven.
    const readable = e.text || e.has_text || (hasText && hasText(e.id));
    if (!readable) continue;

    const user = domains.get(e.domain) ?? { rating: START, n: 0 };
    const target = user.rating + TARGET_OFFSET + shift;
    const rating = items.get(e.id)?.rating ?? seedRating(e);
    const p = expected(user.rating, rating);

    const conceptIds = [e.concept_id, ...(e.topic_concepts ?? [])];
    const ps = conceptIds.flatMap((id) => prereqMastery.get(id) ?? [])
      .filter((x) => x != null);
    const readiness = ps.length ? ps.reduce((a, b) => a + b, 0) / ps.length : 0.5;

    // Freshness: how long ago you last saw it, and how often. A problem served
    // an hour ago is not a good recommendation however well it fits.
    const seen = views.get(e.id)?.ts ?? states.get(e.id)?.last_ts ?? null;
    const ageDays = seen === null ? Infinity : (nowTs - seen) / DAY;
    const staleness = seen === null ? 1 : Math.min(1, ageDays / 14);
    const repeat = 1 + 0.4 * Math.min(4, (views.get(e.id)?.views ?? 0));
    ...
    scored.push({
      ...
      fit: +(Math.abs(rating - target) * (1.3 - 0.3 * readiness)
             * (1 + 2 * (1 - staleness) * repeat)).toFixed(1),
    });
  }

  scored.sort((a, b) => a.fit - b.fit);
  if (!sample) return scored.slice(0, limit);

  // Draw from a band several times the size of the queue, weighted toward the
  // front of it. Everything drawn still fits; which ones you get varies.
  const band = scored.slice(0, Math.max(limit * 8, 40));
  const out = [];
  while (out.length < limit && band.length) {
    const weights = band.map((_, i) => 1 / (i + 4));
    let r = Math.random() * weights.reduce((a, b) => a + b, 0);
    let i = 0;
    while (i < band.length - 1 && (r -= weights[i]) > 0) i += 1;
    out.push(band.splice(i, 1)[0]);
  }
